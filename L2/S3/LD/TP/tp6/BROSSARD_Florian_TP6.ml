...
let (listVarDisj : clause -> string list) = function Disj(l) -> listVar(l);;

(*parcours toutes clause d'une liste de clause pour trouber toutes les variables*)
let rec (listVarDisjList : clause list -> string list) = function
	|	[]			->	[]
	|	tete::queue	->	appendPerso(listVarDisj(tete),listVarDisjList(queue))
	;;
	
let (listVarFormula : cnf -> string list) = function Conj(l) -> elimDoubleStringList(listVarDisjList(l));;
...
let (addResol : string * literal list * literal list * clause list -> clause list) = function v,c,d,l ->
				...
				let res : literal list = appendPerso(a,b) in 
				if (res = []) then failwith "Contradiction trouvée, formule non staisfiable"
				else Disj(res)::l
	;;
...
(*Résultat faux car dpClauseList retourne toujours [] ce qui est équivalent à vrai*)
...
(*
#trace dpVarClauseList;;
#trace dpClauseList;;
#trace dpVarLitListClauseList;;
#trace addResol;;
#trace removeLiteral;;
*)

print_string"f1 = ";printCNF(f1);print_string"\n";;
print_string"Variables de f1 : ";printListVar(listVarFormula(f1));print_string"\n";;
print_string"f2 = ";printCNF(f2);print_string"\n";;
print_string"Variables de f2 : ";printListVar(listVarFormula(f2));print_string"\n";;
print_string"f3 = ";printCNF(f3);print_string"\n";;
print_string"Variables de f3 : ";printListVar(listVarFormula(f3));print_string"\n";;
print_string"f4 = ";printCNF(f4);print_string"\n";;
print_string"Variables de f4 : ";printListVar(listVarFormula(f4));print_string"\n\n";;
