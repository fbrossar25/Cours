...
function winGame(){
  $(".table").html('<span class="winner"><strong>Vous avez réussi !</strong><br>Vous êtes un maître CSS.</span>');
  resetTable();
}
...
function loadBoard(){
  ...
  for(var i = 0;i < boardString.length;i++){
    ...
    if(c == "C") {
      boardMarkup = boardMarkup + '<carrotte></carrotte>\n'
      markup = markup + "<div>&lt;carrotte&gt&lt/carrotte&gt;</div>";
    }
    if(c == "A") {
      boardMarkup = boardMarkup + '<pomme></pomme>\n'
      markup = markup + "<div>&lt;pomme&gt;&lt;/pomme&gt;</div>";
    }
    if(c == "O") {
      boardMarkup = boardMarkup + '<orange></orange>\n'
      markup = markup + "<div>&lt;orange&gt;&lt;/orange&gt;</div>";
    }
    if(c == "P") {
      boardMarkup = boardMarkup + '<cornichon></cornichon>\n'
      markup = markup + '<div>&lt;cornichon&gt;&lt;/cornichon&gt;</div>';
    }
    if(c == "a") {
      boardMarkup = boardMarkup + '<pomme class="petite"></pomme>\n'
      markup = markup + '<div>&lt;pomme class="petite"&gt;&lt;/pomme&gt;</div>';
    }
    if(c == "o") {
      boardMarkup = boardMarkup + '<orange class="petite"></orange>\n'
      markup = markup + '<div>&lt;orange class="petite"&gt;&lt;/orange&gt;</div>';
    }
    if(c == "p") {
      boardMarkup = boardMarkup + '<cornichon class="petite"></cornichon>\n'
      markup = markup + '<div>&lt;cornichon class="petite"&gt;&lt;/cornichon&gt;</div>';
    }
    if(c == "{") {
      boardMarkup = boardMarkup + '<assiette id="fantaisie">'
      markup = markup + '<div>&lt;assiette id="fantaisie"&gt;';
    }
    if(c == "(") {
      boardMarkup = boardMarkup + '<assiette>'
      markup = markup + '<div>&lt;assiette&gt;'
    }
    if(c == ")" || c == "}") {
      boardMarkup = boardMarkup + '</assiette>\n'
      markup = markup + '&lt;/assiette&gt;</div>'
    }
    if(c == "[") {
