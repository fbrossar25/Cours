...
//on compare avec le tableau du mot courant et si le nombre de différences entre eux est égal au nombre de joker
...
void word_array_search_anagrams_wildcard(const struct word_array *self, const char *word, struct word_array *result) {
	...
	if(joker.count > WILDCARDS_MAX){
		fprintf(stderr, "Error : too much wildcard\n");
		return;
	}
	size_t t1[26];
	size_t t2[26];
	...
	for(i=0; i<self->size; i++){//mots du word_array
		...
		if(sz == sz2){
			...
			for(j=0; j<26; j++){//comparaison et réinitialisation du tableau
				if(t2[j] != t1[j]){
					diffCount += abs(t2[j] - t1[j]);
				}
				t2[j] = 0;
			}
			if(diffCount == joker.count){//sont anagrammes
