public class perimetrePolygone {
	static final int N=5; //taille des tableau
	static final double MAX=10;//valeur max de la coordonnées tirés aléatoirement dans random
...
static polygone pointPolyRandom(int n){ //créer des coordonnées aléatoire entre 0 et la constante MAX pour tout les pts du polygone
	int i;
	polygone polyRandom = new polygone();
	for(i=0;i<n;i++){
		polyRandom.tp[i]=new position2D();
	}
	for(i=0;i<=n-1;i++){
		polyRandom.tp[i].x=random(MAX);
		polyRandom.tp[i].y=random(MAX);
	}
	return polyRandom;
}
static void affichagePointPoly(polygone poly){ //affiche les coordonnées de touts les points du polygone
	...
	for(i=0;i<=poly.np-1;i++){
		Ecran.afficher("Point ",i+1," : x=",poly.tp[i].x,"  y=",poly.tp[i].y);
	}
}
